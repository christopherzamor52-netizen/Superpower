...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$DOPERPOWERS_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$DOPERPOWERS_SKILLS_DIR/using-doperpowers/SKILL.md" "$TEST_HOME/using-doperpowers.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$DOPERPOWERS_PLUGIN_FILE" missing
}
