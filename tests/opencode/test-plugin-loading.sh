#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the doperpowers plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/doperpowers.js"
...
skill_count=$(find "$DOPERPOWERS_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $DOPERPOWERS_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-doperpowers skill exists (critical for bootstrap)
echo "Test 3: Checking using-doperpowers skill (required for bootstrap)..."
if [ -f "$DOPERPOWERS_SKILLS_DIR/using-doperpowers/SKILL.md" ]; then
    echo "  [PASS] using-doperpowers skill exists"
else
    echo "  [FAIL] using-doperpowers skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$DOPERPOWERS_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/doperpowers/' "$DOPERPOWERS_PLUGIN_FILE"; then
