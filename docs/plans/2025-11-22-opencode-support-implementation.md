...
> **For agentic workers:** REQUIRED SUB-SKILL: Use doperpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add full doperpowers support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/doperpowers-codex` (lines 40-74)
...
- Reference: `.codex/doperpowers-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'doperpowers' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/doperpowers-codex` (lines 212-280)
 ...
 * (personal skills override doperpowers skills).
 *
 * @param {string} skillName - Name like "doperpowers:brainstorming" or "my-skill"
 * @param {string} doperpowersDir - Path to doperpowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, doperpowersDir, personalDir) {
    // Strip doperpowers: prefix if present
    const forceDoperpowers = skillName.startsWith('doperpowers:');
    const actualSkillName = forceDoperpowers ? skillName.replace(/^doperpowers:/, '') : skillName;

    // Try personal skills first (unless explicitly doperpowers:)
    if (!forceDoperpowers && personalDir) {
    ...
    // Try doperpowers skills
    if (doperpowersDir) {
        const doperpowersPath = path.join(doperpowersDir, actualSkillName);
        const doperpowersSkillFile = path.join(doperpowersPath, 'SKILL.md');
        if (fs.existsSync(doperpowersSkillFile)) {
            return {
                skillFile: doperpowersSkillFile,
                sourceType: 'doperpowers',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/doperpowers-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/doperpowers-codex` (add import at top)
...
Run: `node -c .codex/doperpowers-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/doperpowers-codex
...
**Files:**
- Modify: `.codex/doperpowers-codex` (lines 40-74)
...
Run: `.codex/doperpowers-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/doperpowers-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/doperpowers-codex` (lines 16-38, approximately)
...
Run: `.codex/doperpowers-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/doperpowers.js`
...
/**
 * Doperpowers plugin for OpenCode.ai
 *
...
const doperpowersSkillsDir = path.join(homeDir, '.config/opencode/doperpowers/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const DoperpowersPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/doperpowers.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/doperpowers.js
...
**Files:**
- Modify: `.opencode/plugin/doperpowers.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "doperpowers:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > doperpowers)
          const resolved = skillsCore.resolveSkillPath(
            skill_name,
            doperpowersSkillsDir,
            personalSkillsDir
          );
...
Run: `node -c .opencode/plugin/doperpowers.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the doperpowers and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const doperpowersSkills = skillsCore.findSkillsInDir(
            doperpowersSkillsDir,
            'doperpowers',
            3
          );
          ...
          const allSkills = [...personalSkills, ...doperpowersSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install doperpowers skills to ~/.config/opencode/doperpowers/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'doperpowers:';
...
```javascript
    'session.started': async () => {
      // Read using-doperpowers skill content
      const usingDoperpowersPath = skillsCore.resolveSkillPath(
        'using-doperpowers',
        doperpowersSkillsDir,
        personalSkillsDir
      );

      let usingDoperpowersContent = '';
      if (usingDoperpowersPath) {
        const fullContent = fs.readFileSync(usingDoperpowersPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingDoperpowersContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Doperpowers skills: \`doperpowers:skill-name\` (from ~/.config/opencode/doperpowers/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/doperpowers')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/doperpowers && git pull` to update doperpowers.' :
        '';

      // Return context to inject into session
      return {
        context: `<EXTREMELY_IMPORTANT>
You have doperpowers.

**Below is the full content of your 'doperpowers:using-doperpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingDoperpowersContent}
...
```markdown
# Installing Doperpowers for OpenCode
...
### 1. Install Doperpowers Skills

```bash
# Clone doperpowers skills to OpenCode config directory
mkdir -p ~/.config/opencode/doperpowers
git clone https://github.com/obra/superpowers.git ~/.config/opencode/doperpowers
```

### 2. Install the Plugin

The plugin is included in the doperpowers repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/doperpowers/.opencode/plugin/doperpowers.js`
...
ln -s ~/.config/opencode/doperpowers/.opencode/plugin/doperpowers.js .opencode/plugin/doperpowers.js
```
...
```
use use_skill tool with skill_name: "doperpowers:brainstorming"
```
...
Personal skills override doperpowers skills with the same name.

## Updating

```bash
cd ~/.config/opencode/doperpowers
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/doperpowers/.opencode/plugin/doperpowers.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/doperpowers/skills`
...
Doperpowers works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/doperpowers-codex use-skill doperpowers:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/doperpowers-codex bootstrap | head -30`
...
ls -l .opencode/plugin/doperpowers.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/doperpowers.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/doperpowers-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/doperpowers-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/doperpowers.js` created with tools and hooks
