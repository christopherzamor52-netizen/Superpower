...
Add full doperpowers support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port doperpowers to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/doperpowers-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/doperpowers.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/doperpowers/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/doperpowers-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "doperpowers:brainstorming")')
  }),
...
1. **Inject using-doperpowers content**
   - Full content of the using-doperpowers skill
...
```javascript
// .opencode/plugin/doperpowers.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const DoperpowersPlugin = async ({ client, directory, $ }) => {
  const doperpowersDir = path.join(process.env.HOME, '.config/opencode/doperpowers');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingDoperpowers = await readSkill('using-doperpowers');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingDoperpowers}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── doperpowers-codex        # UPDATED: Use skills-core
│   ├── doperpowers-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── doperpowers.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/doperpowers-codex`
...
2. Update `.codex/doperpowers-codex` to use shared core
...
1. Create `.opencode/plugin/doperpowers.js`
