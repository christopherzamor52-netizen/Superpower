...
- **Controlled track**: taste-heavy, novel, or high-stakes work where design judgment keeps arising mid-flight → superpowers:brainstorming.
...
Save to `docs/superpowers/execplans/YYYY-MM-DD-<topic>.md` (omit the triple-backtick envelope per PLANS.md's file rule). The bar: a fresh session with no conversation history — or a daemon spawned with nothing but this file — can implement it end-to-end and see it working.

## Step 3 — Execute

In an isolated workspace (superpowers:using-git-worktrees). Follow PLANS.md's implementing contract as written: do not prompt your human partner for next steps; resolve ambiguities autonomously (the grill already exhausted the ones that needed a human); keep `Progress`, `Surprises & Discoveries`, and the `Decision Log` current at every stopping point; commit frequently.

This profile fits durable background daemons (superpowers:orchestrating-daemons): the ExecPlan is exactly what a spawn prompt can carry, and it survives the daemon's context death — the document is the memory.

## Exit gate

Exactly one, at the end. Before merging: dispatch the final whole-branch review per superpowers:requesting-code-review, then finish with superpowers:finishing-a-development-branch. Its retrospective step writes into the ExecPlan's own `Outcomes & Retrospective` section — the ExecPlan is this track's spec-equivalent.
