...
# 1. Create in doperpowers location (lowest priority)
mkdir -p "$DOPERPOWERS_SKILLS_DIR/priority-test"
cat > "$DOPERPOWERS_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Doperpowers version of priority test skill
---
# Priority Test Skill (Doperpowers Version)

This is the DOPERPOWERS version of the priority test skill.

PRIORITY_MARKER_DOPERPOWERS_VERSION
EOF
...
if [ -f "$DOPERPOWERS_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Doperpowers version exists"
else
    echo "  [FAIL] Doperpowers version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled doperpowers priority
echo ""
echo "Test 2: Documenting personal vs doperpowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled doperpowers version instead of personal version"

# Test 3: Document project vs bundled doperpowers priority
echo ""
echo "Test 3: Documenting project vs personal/doperpowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled doperpowers version instead of project version"

# Test 4: Test a non-colliding bundled doperpowers skill is still available
echo ""
echo "Test 4: Testing non-colliding doperpowers skill remains available..."

mkdir -p "$DOPERPOWERS_SKILLS_DIR/doperpowers-only-test"
cat > "$DOPERPOWERS_SKILLS_DIR/doperpowers-only-test/SKILL.md" <<'EOF'
---
name: doperpowers-only-test
description: Doperpowers-only priority test skill
---
# Doperpowers Only Test Skill

PRIORITY_MARKER_DOPERPOWERS_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"doperpowers-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_DOPERPOWERS_ONLY_VERSION" "Non-colliding doperpowers skill is still registered"
