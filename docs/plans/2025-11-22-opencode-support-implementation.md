...
> **For agentic workers:** REQUIRED SUB-SKILL: Use ace:executing-plans to implement this plan task-by-task.
 ...
 *
 * @param {string} skillName - Name like "ace:brainstorming" or "my-skill"
 * @param {string} superpowersDir - Path to superpowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, superpowersDir, personalDir) {
    // Strip ace: prefix if present
    const forceSuperpowers = skillName.startsWith('ace:');
    const actualSkillName = forceSuperpowers ? skillName.replace(/^ace:/, '') : skillName;

    // Try personal skills first (unless explicitly ace:)
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "ace:brainstorming" or "my-custom-skill")')
        }),
...
```javascript
      {
        ...
        execute: async () => {
          ...
          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'ace:';
...
**Skills naming:**
- Superpowers skills: \`ace:skill-name\` (from ~/.config/opencode/superpowers/skills/)
...
**Below is the full content of your 'ace:using-superpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
```
use use_skill tool with skill_name: "ace:brainstorming"
```
...
Run: `.codex/superpowers-codex use-skill ace:brainstorming | head -20`
