...
main() {
    ...
    if [[ "$dir" == "$repo/.doperpowers/sdd" ]]; then
        pass "prints <repo-root>/.doperpowers/sdd"
    else
        fail "prints <repo-root>/.doperpowers/sdd"
        echo "    got: $dir"
    fi
    ...
    printf 'x\n' > "$repo/.doperpowers/sdd/artifact.md"
    local status
...
PLAN
    ...
    case "$brief_path" in
        "$repo/.doperpowers/sdd/"*) pass "task-brief writes its brief under the workspace" ;;
        *)
    ...
    case "$rp_path" in
        "$repo/.doperpowers/sdd/"*) pass "review-package writes its diff under the workspace" ;;
        *)
    ...
    if [[ "$wt_dir" == "$wt_root/.doperpowers/sdd" && "$wt_dir" != "$dir" ]]; then
    ...
    printf 'y\n' > "$wt/.doperpowers/sdd/artifact.md"
    local wt_status
